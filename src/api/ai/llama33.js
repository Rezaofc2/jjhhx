...
module.exports = (app) => {
  async function metaai(prompt,text) {
    try {
      // Menggunakan template literal yang benar dengan backticks (``)
      const response = await axios.get(`https://api.siputzx.my.id/api/ai/llama33?prompt=${prompt}&text=${text}`);
  ...
  app.get("/ai/metai", async (req, res) => {
    ...
    try {
      const result = await metaai(prompt, text); // Kirim parameter ke fungsi gemini
      res.status(200).json({
        status: true,
        result,
      });
